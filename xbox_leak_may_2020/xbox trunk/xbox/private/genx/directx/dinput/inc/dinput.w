 ...
 *  Copyright (C) 1996-2000 Microsoft Corporation.  All Rights Reserved.;public_dx8
 *
 *  File:       dinput.h
 ...
 *   2000.05.10 mitchd   Forked for XBOX
 *****************************************************************************
 *	Note on XBOX Fork: We want to preserve this file as much as possible.
 *	A) We never know when we might want to suck something
 *     into XBOX that we previously excluded.
 *  B) We want to make it easy to resync with updates to the original headers
 *     made by the Windows DInput team.
 *  C) We want to make it easy for the DInput team to suck over the new additions
 *     into DInput for Windows.
 *
 *  Marker: ;public_dx8x
 *    New additions for xbox DX8 only.
 *   	
 *  Marker: ;noxbox
 *    Is used to mark material to be excluded from the xbox headers.
...
;begin_noxbox
#define DIRECTINPUT_HEADER_VERSION  0x070F                      ;public_800
...
begindoc
*
*	Class IDs are not defined for XBOX, as XBOX does not support
*	CoCreateInstance, or COM as we know it.
*
enddoc 
 ...
 *
 *
 *		Xbox does not support legacy interfaces and is unicode only.
 *		So we only define the class IDs for the appropriate xbox version
 *		and only the unicode version.
 *
enddoc
 ****************************************************************************/
;begin_noxbox
...
begindoc
*
*	We don't have predefined product guids on xbox
*
enddoc
;begin_noxbox
/****************************************************************************
 ...
 ****************************************************************************/
/*
 *          BUGBUG -- Which of the basic types support envelopes?
 ...
 *          custom effects.
 *
 *          BUGBUG -- enforce the rule on channels and direction.
 *
 ...
 *          the minimal semantic mapper joystick device capabilities.
 *
 *          <c DI8DEVTYPEJOYSTICK_ENHANCED>: A joystick which provides the 
 *          minimal semantic mapper joystick device capabilities plus more 
 *          axes and/or hat switches.
 *
 ...
 *
 *          <c DI8DEVTYPEDEVICECTRL_COMMSSELCTION>: A control used to make 
...
;end
;begin_if_(DIRECTINPUT_VERSION)_701
...
#define DIA_VALID               0x0000000F  ;internal_800
 ...
 *  @flag   DIAH_OTHERAPP |
 *
 *          The user specified this control for the same semantic in a similar
 *          application.
 *
 *  @flag   DIAH_ERROR |
 *
 ...
 *          relative to the upper left corner of the configuration image.
 *
 *  @flag   DIDIFT_CONTROL | 
 *
 *          The image info is an overlay for a configuration image. The 
 *          <e DIDEVICEIMAGEINFO.dwOverlayOffset>, 
 ...
 *
 *  @flag   DIDIFT_SELECTION | 
 *
 *          The file is a device selection image. It is intended to be 
 *          presented to users to enable them to choose a single device 
 *          among several installed devices.
 *
 *  @flag   DIDIFT_VIEWSELECT | 
 *
 *          The file is a view selection image. It is used by devices which 
 *          require multiple views to properly display all controls on the 
 *          device. 
 *
 *  @flag   DIDIFT_IMAGE2D_PNG | 
 *
 *          The file referred to by lptszImage uses the PNG image format. 
 *
 *  @doc    INTERNAL
 *
 *  @define DIDIFT_IMAGE2D_BMP | 0x80000000 |
 *
 *          Internal flag in <e DIDEVICEIMAGEINFO.dwFlags> to 
 *          indicate that the file referred to by 
 *          <e DIDEVICEIMAGEINFO.lptszImagePath> uses the BMP image format. 
 *
 ****************************************************************************/
enddoc
...
#define DIDIFTT_VALID           0x0000000F ;internal
#define DIDIFT_IMAGE2D_PNG      0x00010000
#define DIDIFT_IMAGE2D_BMP      0x80000000 ;internal
#define DIDIFTF_VALID           0x80010000 ;internal
#define DIDIFT_VALID            ( DIDIFTT_VALID | DIDIFTF_VALID );internal
 ...
 *
 *  @field  DWORD | dwOverlayOffset |
 *
 *          Offset of the device configuration image over which this overlay 
 *          is to be displayed. 
...
;begin_internal
 *          BUGBUG if this flag is set, can you specify a <t DIREGION>
 ...
 *
 *          Generic structure used to access properties whose values
 *          represent calibration information.
 *
 *  @field  DIPROPHEADER | diph |
 *
 *          (IN) "Must" be preinitialized as follows:
 ...
 *
 *          BUGBUG -- DirectInput control panel doesn't do this yet.
 *
 *  @devnote
 *
 *          This is new for DX5.
...
begindoc
/****************************************************************************
 *  BUGBUG MarcAnd: This is my guess at the property
 *
 *  @doc    EXTERNAL
 *
 ...
 *  @doc    EXTERNAL
 *
 *  @define DIPROP_TYPEKEYNAME | MAKEDIPROP(26) |
 *
 *          Predefined property which retrieves the registry key name under 
 ...
 *  @devnote
 *
 *          The <c DIPROP_TYPEKEYNAME> property is new for DirectX 8
 *
 ****************************************************************************/
enddoc
#define DIPROP_TYPEKEYNAME      MAKEDIPROP(26)
 ...
 *
 *          Predefined property which retrieves the device's
 *          player slot.  This setting applies to the entire device, rather
 *          than to any particular object, so the <e DIPROPHEADER.dwHow>
 *			 field must be <c DIPH_DEVICE>.
 *
 *          The <p pdiph> "must" be a
 *          pointer to the <e DIPROPDWORD.diph> member of a
 *          <t DIPROPDWORD> structure.
 ...
 *  @devnote
 *
 *          The <c DIPROP_PLAYERSLOT> property is new for XBOX
 *
 ****************************************************************************/
enddoc
#define DIPROP_PLAYERSLOT                 MAKEDIPROP(128)
 ...
 *
 *          Predefined property to set whether devices polls in the background
 *			or requires a call to the <mf IDirectInputDevice7::Poll> to get
 *          fresh data.  This setting applies to the entire device, rather
 *          than to any particular object, so the <e DIPROPHEADER.dwHow>
 *			 field must be <c DIPH_DEVICE>.
 *
 *          The <p pdiph> "must" be a
 ...
 *  @devnote
 *
 *          The <c DIPROP_AUTOPOLL> property is new for XBOX
 *
 ****************************************************************************/
enddoc
#define DIPROP_AUTOPOLL                   MAKEDIPROP(129)
 ...
 *  @doc    INTERNAL
 *
 *  @func   VOID | INITINSTANCEGUID |
 *
 *          Helper macro to initialize a guidInstance from an ID
 *
 *          Set guidInstance.Data1 to the ID, everything else to 0.
 *          Xbox doesn't really require guids for instances.  This
 *          is just a convenient way not to change the protocol.
 *
 *  @parm   guidInstance - variable to initialize.
 *  @parm   bDeviceId - Id to set it to.
 *
 *
 ****************************************************************************
 *
 *  @doc    INTERNAL
 *
 *  @func   BOOL | IS_VALID_GUID_INSTANCE |
 *
 *          Helper macro to check that guidInstance is valid.
 *          Basically that only guidInstance.Data1 is non-Zero.
 *
 *  @parm   guidInstance - variable to check.
 *
 *  @rdesc  Either:
 *  @flag   TRUE - If a valid guidInstance.
 *  @flag   FALSE - If not a valid guidInstance.
 *
 ****************************************************************************/
 *
 *  @doc    INTERNAL
 *
 *  @func   BYTE | DEVICEIDFROMGUID |
 *
 *          Helper macro to retrieve the device ID from an guidInstance.
 *          Assumes that the guidInstance is valid.
 *
 *  @parm   guidInstance - variable to check.
 *
 *  @rdesc  Device ID
 ...
 *  @doc    INTERNAL
 *
 *  @struct DIXBOXCONTROLLERSTATE |
 *
 *          Instantaneous game controller status for
 *          the base XBOX controller.
 *
 *  @field  WORD | Reserved |
 *
 *          (OUT) Internal Use.
 *
 *  @field  WORD | bmButtons |
 *
 *          (OUT) Bitmap of status of up to 16 digital buttons.
 *
 *  @field  BYTE | rgbAnalogButtons[8] |
 *
 *          (OUT) Array of proportional button states.
 *
 *  @field  WORD | wDPAD |
 *
 *          (OUT) Position of DPAD.  This reports like a POV.
 *			A value of 0xFFFF indicates centered.  Other values
 *			indicate angle from North in 100ths of a degree.
 *
 *  @field  WORD | wThumb1X |
 *
 *          (OUT) X axis position of the left most
 *          thumbstick control.
 *
 *  @field  WORD | wThumb1Y |
 *
 *          (OUT) Y axis position of the left most
 *          thumbstick control.
 *
 *  @field  WORD | wThumb2X |
 *
 *          (OUT) X axis position of the right most
 *          thumbstick control.
 *
 *  @field  WORD | wThumb2Y |
 *
 *          (OUT) Y axis position of the right most
 *          thumbstick control.
 *
 *  @field  WORD | wTiltX |
 *
 *          (OUT) X axis tilt of the  control.
 *
 *  @field  WORD | wTiltY |
 *
 *          (OUT) Y axis tilt of the  control.
 *
 *****************************************************************************/
enddoc
 ...
 *          Internal Use.
 *
 *  @field  BYTE | bLeftMotorSpeed |
 *
 *          New speed for left motor, 0 is off, 255 is fastest available speed.
 *
 *  @field  BYTE | bRightMotorSpeed |
 *
 *         New speed for right motor, 0 is off, 255 is fastest available speed.
 *
 *
 *****************************************************************************/
enddoc
 ...
 *
 *          All installed devices for the user identified by ptszUserName, 
 *          and all unowned devices, are enumerated. 
 *
