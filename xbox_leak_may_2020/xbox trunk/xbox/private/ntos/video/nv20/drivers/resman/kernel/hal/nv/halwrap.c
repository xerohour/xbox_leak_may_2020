...
RM_STATUS
nvHalGrAlloc(PHWINFO pDev, U032 chid, U032 instance, U032 classNum, U032 flags)
{
...
RM_STATUS
nvHalDacProgramMClk(PHWINFO pDev, U032 DisableDLLs)
{
    ...
    dacProgramMClkArg.DisableDLLs = DisableDLLs;
...
RM_STATUS
nvHalDacValidateBandwidth(PHWINFO pDev, U032 VidScalerHead, BANDWIDTH_CALC_PARAMS* head0, BANDWIDTH_CALC_PARAMS* head1)
{
    DACVALIDATEBANDWIDTHARG_000   dacValidateBandwidthArg;
    RM_STATUS status;

    dacValidateBandwidthArg.id = DAC_VALIDATEBANDWIDTH_000;
    dacValidateBandwidthArg.pHalHwInfo = &pDev->halHwInfo;
    dacValidateBandwidthArg.VidScalerHead = VidScalerHead;
    dacValidateBandwidthArg.head0 = head0;
    dacValidateBandwidthArg.head1 = head1;

    // fills in dacValidateArbSettingsArg.result
    status = HALFUNC_000(_nvHalDacValidateBandwidth)((VOID *)&dacValidateBandwidthArg);
...
RM_STATUS
nvHalDacValidateArbSettings(PHWINFO pDev, U032 bpp, U032 video_enabled, U032 mport_enabled, U032 vclk, U032 head, U032 *result)
{
...
RM_STATUS
nvHalDacHotPlugInterrupts(PHWINFO pDev, VOID * pHalObj, U032 * HotPlugStatus, U032 * HotUnplugStatus, BOOL ReadWrite, BOOL Reset)
{
    DACHOTPLUGINTERRUPTS_000    dacHotPlugInterruptsArg;
    RM_STATUS halStatus;
    
    dacHotPlugInterruptsArg.id=DAC_HOTPLUGINTERRUPTS_000;
    dacHotPlugInterruptsArg.pHalHwInfo = &pDev->halHwInfo;
    dacHotPlugInterruptsArg.pHalObjInfo = pHalObj;
    dacHotPlugInterruptsArg.HotPlugStatus = *HotPlugStatus;
    dacHotPlugInterruptsArg.HotUnplugStatus = *HotUnplugStatus;
    dacHotPlugInterruptsArg.ReadWrite = ReadWrite;
    dacHotPlugInterruptsArg.Reset = Reset;
    
    halStatus=(HALFUNC_000(_nvHalDacHotPlugInterrupts)((VOID *)&dacHotPlugInterruptsArg));
    
    if(ReadWrite==FALSE)
    {
        //read mode, we need to transfer the values back
        *HotPlugStatus = dacHotPlugInterruptsArg.HotPlugStatus;
        *HotUnplugStatus = dacHotPlugInterruptsArg.HotUnplugStatus;
    }
