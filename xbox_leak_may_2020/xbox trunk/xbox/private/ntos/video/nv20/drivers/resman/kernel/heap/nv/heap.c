...
// Globals.
//
static U032 memTypeFlags[NUM_MEM_TYPES+1] =
{
    MEM_FLAG_IMAGE,
...
)
{
    ...
    if (!heap) return;
    FIFO_PRINTF((DBG_LEVEL_INFO,"Heap dump.  Size = 0x%x\n", heap->total));
    FIFO_PRINTF((DBG_LEVEL_INFO,"            Free = 0x%x\n", heap->free));
    FIFO_PRINTF((DBG_LEVEL_INFO,"=================================================================\r\n"));
    for (i = 0; i < heap->numBanks; i++)
    {
        FIFO_PRINTF((DBG_LEVEL_INFO,"Bank: 0x%x\n", i));
        FIFO_PRINTF((DBG_LEVEL_INFO,"\tType flags = 0x%x\n", heap->Bank[i].flags));
        FIFO_PRINTF((DBG_LEVEL_INFO,"\tOffset     = 0x%x\n", heap->Bank[i].offset));
        FIFO_PRINTF((DBG_LEVEL_INFO,"\tSize       = 0x%x\n", heap->Bank[i].size));
    }
    FIFO_PRINTF((DBG_LEVEL_INFO,"Block List Forward:\r\n"));
    block = heap->pBlockList;
    do
    {
        if (block->owner == FREE_BLOCK)
            FIFO_PRINTF((DBG_LEVEL_INFO,"\tOwner = FREE\r\n"));
        else
        {
            FIFO_PRINTF((DBG_LEVEL_INFO,"\tOwner = 0x%x\n", block->owner));
            FIFO_PRINTF((DBG_LEVEL_INFO,"\tType  = 0x%x\n", block->u0.type));
            FIFO_PRINTF((DBG_LEVEL_INFO,"\tHwres = 0x%x\n", block->u1.hwres));
        }
        FIFO_PRINTF((DBG_LEVEL_INFO,"\t\tBegin = 0x%x\n", block->begin));
        FIFO_PRINTF((DBG_LEVEL_INFO,"\t\tEnd   = 0x%x\n", block->end));
        block = block->next;
    } while (block != heap->pBlockList);
    FIFO_PRINTF((DBG_LEVEL_INFO,"Block List Reverse:\r\n"));
    block = heap->pBlockList;
    do
    {
        block = block->prev;
    ...
    FIFO_PRINTF((DBG_LEVEL_INFO,"FREE Block List Forward:\r\n"));
    free  = 0;
    ...
    FIFO_PRINTF((DBG_LEVEL_INFO,"\tCalculated free count = 0x%x\n", free));
    FIFO_PRINTF((DBG_LEVEL_INFO,"FREE Block List Reverse:\r\n"));
    free  = 0;
    if ((block = heap->pFreeBlockList))
        do
        {
...
)
{
    ...
    do
    {
        if (block->owner == FREE_BLOCK)
        {
            if (!blockFree)
            {
                FIFO_PRINTF((DBG_LEVEL_ERRORS,"Validate: Invalid free list with free blocks found.\r\n"));
                heapDump(heap);
                DBG_BREAKPOINT();
            }
            free += block->end - block->begin + 1;
            if (block != blockFree)
            {
                FIFO_PRINTF((DBG_LEVEL_ERRORS,"Validate: Free list not consistent with block list.\r\n"));
                heapDump(heap);
                DBG_BREAKPOINT();
            }
        ...
        if (block->next != heap->pBlockList)
        {
            ...
            {
                FIFO_PRINTF((DBG_LEVEL_ERRORS,"Validate: Hole between blocks at offset = 0x%x\n", block->end));
                heapDump(heap);
                DBG_BREAKPOINT();
            }
        }
        else
        {
            ...
            {
                FIFO_PRINTF((DBG_LEVEL_ERRORS,"Validate: Last block doesn't end at top.\r\n"));
                heapDump(heap);
                DBG_BREAKPOINT();
            }
            if (block->next->begin != 0)
            {
                FIFO_PRINTF((DBG_LEVEL_ERRORS,"Validate: First block doesn't start at bottom.\r\n"));
                heapDump(heap);
                DBG_BREAKPOINT();
            }
        }
        ...
        {
            FIFO_PRINTF((DBG_LEVEL_ERRORS,"Validate: Invalid block begin = 0x%x\n", block->begin));
            FIFO_PRINTF((DBG_LEVEL_ERRORS,"                        end   = 0x%x\n", block->end));
            heapDump(heap);
            DBG_BREAKPOINT();
        }
        block = block->next;
    ...
    if (free != heap->free)
    {
        FIFO_PRINTF((DBG_LEVEL_ERRORS,"Validate: Calculated free count not consistent with heap free count.\r\n"));
        heapDump(heap);
        DBG_BREAKPOINT();
    }
    if ((used + free) > heap->total)
    {
        FIFO_PRINTF((DBG_LEVEL_ERRORS,"Validate: Calculated used count not consistent with heap size.\r\n"));
        heapDump(heap);
        DBG_BREAKPOINT();
    }
}
...
)
{
    ...
    //
    FIFO_PRINTF((DBG_LEVEL_INFO, "NVRM: Heap Manager: HEAP ABOUT TO BE CREATED.\r\n"));
...
)
{
    ...
    FIFO_PRINTF((DBG_LEVEL_INFO, "NVRM: Heap Manager: HEAP ABOUT TO BE DESTROYED.\r\n"));
    ...
    // If the "internal" interface is indicated, free *all* allocated blocks
    //
    do {
        ...
        do {
            ...
            if ( (internal) || ((block->owner != FREE_BLOCK) && 
...
//
// Our aligment constraints may not involve a ^2, so we have
// to use integer arithmetic to get the right block start address.
//
// XXXscottl
// We don't need these (yet).
//
#define BLOCK_ALIGN_BOTTOM(b,o)         (((b+(o))/(o+1))*(o+1))
#define BLOCK_ALIGN_TOP(b,o)            (((b)/(o+1))*(o+1))
...
{
    U032      alignment, alignmentPerf, i, typeFlags, headNum;
    ...
    *offset = 0;
    ...
    if (status != RM_OK && status != RM_ERR_INSUFFICIENT_RESOURCES)
    {
        ...
        FIFO_PRINTF((DBG_LEVEL_ERRORS, "NVRM: nvHalFbAlloc failure!\n"));
        return (RM_ERROR);
    }
    ...
    *size = pFbAllocInfo->size;
    ...
    alignmentPerf = pFbAllocInfo->alignPerf;
            ...
            do
            {
                if ((blockFree->begin <= bankHi) && (blockFree->end >= bankLo))
                {
                    ...
                    if (heap->Bank[i].flags & MEM_GROW_DOWN)
                    {
                        ...
                        //allocLo = BLOCK_ALIGN_TOP(freeHi-*size+1, alignmentPerf);
                        allocAl = allocLo;
                        allocHi = freeHi;
                    }
                    else
                    {
                        ...
                        //allocAl = BLOCK_ALIGN_BOTTOM(freeLo, alignmentPerf);
                    ...
                    if ((allocLo == blockFree->begin) && (allocHi == blockFree->end))
                    {
                        ...
                        *offset     = blockFree->align;
                        ...
                        if (type == MEM_TYPE_PRIMARY)
                        {
                            pDev->Framebuffer.PrimarySurfaceAlloced++;
    ...
    do
    {
        if (typeFlags & MEM_GROW_DOWN)
        {
            ...
            //allocLo   = BLOCK_ALIGN_TOP(blockFree->end-*size+1, alignment);
            allocAl   = allocLo;
        ...
        else
        {
            ...
            //allocAl   = BLOCK_ALIGN_BOTTOM(blockFree->begin, alignment);
            allocHi   = allocAl + *size - 1;
        }
        //
        // Check for wrap-around.
