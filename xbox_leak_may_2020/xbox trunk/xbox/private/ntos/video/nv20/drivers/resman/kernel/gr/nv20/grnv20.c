...
)
{
    ...
    U032 dmaPush = 0;           // init to keep compiler from complaining
    U032 i;

    grCtxBuffer = pGrHalPvtInfo->CtxTable[ChID];

    GR_PRINTF((DBG_LEVEL_INFO, "NVRM: initializing 3d state: chid: 0x%x\n", ChID));
    GR_PRINTF((DBG_LEVEL_INFO, "      buffer offset: 0x%x\n", grCtxBuffer));
    GR_PRINTF((DBG_LEVEL_INFO, "      buffer size: 0x%x\n", sizeof (GRAPHICSCHANNEL_NV20)));
    ...
    {
        GR_PRINTF((DBG_LEVEL_INFO, "NVRM: unload current chid: 0x%x\n", ChID));
    ...
    GR_PRINTF((DBG_LEVEL_INFO, "      pipe offset: 0x%x\n", offset));
    ...
    GR_PRINTF((DBG_LEVEL_INFO, "      end pipe offset: 0x%x\n", offset));
    ...
    GR_PRINTF((DBG_LEVEL_INFO, "      last offset: 0x%x\n", offset));
    ...
    if (pGrHalPvtInfo->currentChID == ChID)
    {
        GR_PRINTF((DBG_LEVEL_INFO, "NVRM: reload current chid: 0x%x\n", ChID));
