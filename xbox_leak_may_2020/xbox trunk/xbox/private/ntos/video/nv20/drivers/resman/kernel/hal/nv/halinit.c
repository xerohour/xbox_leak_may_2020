...
initHal(PHWINFO pDev)
{
    ...
    //
    // Setup the dynamic code loader.  
    // This isn't necessarily tied to the HAL as we could later
    // choose to patch more than just the HAL.  If that becomes the case
    // we possibly need to move this elsewhere.  initDload is smart
    // enough to remain idempotent across multiple invocations to 
    // the same board, and it is, in fact, not tied to any particular board.
    // [ It hangs its state off of RMINFO. ]
    //
    if ( initDload(pDev) != RM_OK ) 
    {
	    DBG_PRINT_STRING(DEBUGLEVEL_ERRORS,"NVRM: *** Cannot initialize dynamic code load state\n");
	    return FALSE;
    }
    ...
    else if (MC_GET_ARCHITECTURE(pmcBoot0) == MC_ARCHITECTURE_NV10)
    {
        ...
        } else if (MC_GET_IMPLEMENTATION(pmcBoot0) == MC_IMPLEMENTATION_NV1A) {
            pDev->classDB.pClassDescriptors =   NV11_ClassDescriptors;
            pDev->classDB.numClassDescriptors = NV11_NumClassDescs;
        }
    }
