...
#ifndef DXAPI
    // For the XBOX, we need to assign all DLS collections that are used by this track to the
    // synth. Endplay will disconnect them.
    IDirectMusicSynthX *pSynth;
    if (SUCCEEDED(pPerformance->GetSynth(&pSynth)))
    {
        for (;pBand;pBand = pBand->GetNext())
        {
            ...
            {
                if (pInstrument->m_pIDMCollection)
                {
                    pSynth->AssignDLS(pInstrument->m_pIDMCollection);
                }
            }
        }
        pSynth->Release();
...
#ifndef DXAPI
    // For the XBOX, we need to disconnect all DLS collections that are used by this track 
    // from the synth. 
    IDirectMusicSynthX *pSynth;
    CBandTrkStateData* pBandTrkStateData = (CBandTrkStateData *) pStateData;
    if (SUCCEEDED(pBandTrkStateData->m_pPerformance->GetSynth(&pSynth)))
    {
        CBand* pBand = BandList.GetHead();
        for (;pBand;pBand = pBand->GetNext())
        {
            ...
            for (;pInstrument;pInstrument = pInstrument->GetNext())
            {
                if (pInstrument->m_pIDMCollection)
                {
                    pSynth->UnassignDLS(pInstrument->m_pIDMCollection);
                }
            }
        }
        pSynth->Release();
