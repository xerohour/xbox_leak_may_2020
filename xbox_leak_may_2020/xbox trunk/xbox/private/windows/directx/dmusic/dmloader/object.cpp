...

{
    ...
    if (m_ObjectDesc.m_dwValidData & DMUS_OBJ_FILENAME)
    {
        ...
        {
            hr = m_pClass->GetPath(wzFullPath);
            if (SUCCEEDED(hr)) {
                wcscat(wzFullPath,m_ObjectDesc.m_pwzFileName);
            } else {
#ifdef DBG
                DebugTrace(1,"DMUSIC:Warning SetSearchDirectory has not been called");
#endif
            }
        }
...
HRESULT CClass::SetSearchDirectory(const WCHAR * pwzPath,BOOL fClear)

{
    HRESULT hr;

    hr = m_ClassDesc.SetFileName((WCHAR *)pwzPath);
    if (SUCCEEDED(hr))
    {
