...
union EventUnion {
    ...
    Buffer_SetFormat bufferSetFormat;
...
public:
    ...
    STDMETHOD(SetFormat)           (THIS_ REFERENCE_TIME timeStamp, LPCWAVEFORMATEX pwfxFormat);
...
HRESULT
Buffer::SetFormat(REFERENCE_TIME timeStamp, LPCWAVEFORMATEX pwfxFormat){
    EventArgs eventArgs;
    eventArgs.timeStamp = timeStamp;
    eventArgs.eventType = eBufferSetFormat;
    memcpy(&eventArgs.body.bufferSetFormat.wfx, pwfxFormat, sizeof(*pwfxFormat) + pwfxFormat->cbSize);
    return Enqueue(&eventArgs);
}
...
Buffer::DispatchEvent(EventArgs* pEventArgs){
    ...
    case eBufferSetFormat:
        EACH_CALL_DEBUGF(("Buffer[%03d]->SetFormat(0x%08x)\n", m_dwid,
            &pEventArgs->body.bufferSetFormat.wfx
            ));
        hr = m_pObject->SetFormat(&pEventArgs->body.bufferSetFormat.wfx.wfx
            );
        break;
...
Stream::Stream(){
    ...
    memset(m_dwPacketStatus,0,sizeof(m_dwPacketStatus));
...
HRESULT
Stream::SetFormat(REFERENCE_TIME timeStamp, LPCWAVEFORMATEX pwfxFormat){
    EventArgs eventArgs;
    eventArgs.timeStamp = timeStamp;
    eventArgs.eventType = eStreamSetFormat;
    memcpy(&eventArgs.body.bufferSetFormat.wfx, pwfxFormat, sizeof(*pwfxFormat) + pwfxFormat->cbSize);
    return Enqueue(&eventArgs);
}
...
Stream::DispatchEvent(EventArgs* pEventArgs){
    ...
    case eStreamSetFormat:
        EACH_CALL_DEBUGF(("Stream[%03d]->SetFormat(0x%08x)\n", m_dwid,
            &pEventArgs->body.bufferSetFormat.wfx
            ));
        hr = m_pObject->SetFormat(&pEventArgs->body.bufferSetFormat.wfx.wfx
            );
