...
STDMETHODIMP CSynth::AssignDLS(IDirectMusicCollection *pCollection)

{
    ...
    if (pCollection)
    {
        ...
        if (SUCCEEDED(pCollection->QueryInterface(IID_CCollection,(void **)&pCCollection)))
        {
            if (pCCollection->m_pSynth == NULL)
            {
                m_Instruments.m_CollectionList.AddHead(pCCollection);
                pCCollection->m_dwConnectionCount = 1;
                pCCollection->m_pSynth = this;
            }
            else if (pCCollection->m_pSynth == this)
            {
                pCCollection->m_dwConnectionCount++;
            }
            else
            {
                hr = E_FAIL;
                Trace(0,"Error: Attempted to play DLS Collection on more than one synth.\n");
            }
            pCollection->Release();
        }
    }
...
STDMETHODIMP CSynth::UnassignDLS(IDirectMusicCollection *pCollection)

{
    ...
    if (pCollection)
    {
        ...
        if (SUCCEEDED(pCollection->QueryInterface(IID_CCollection,(void **)&pCCollection)))
        {
            if (pCCollection->m_pSynth == this)
            {
                pCCollection->m_dwConnectionCount--;
                if (!pCCollection->m_dwConnectionCount)
                {
                    m_Instruments.m_CollectionList.Remove(pCCollection);
                    pCCollection->m_pSynth = NULL;
                }
            }
            else
            {
                hr = E_FAIL;
                Trace(0,"Error: Attempted to disconnect DLS Collection from incorrect synth.\n");
            }
            pCollection->Release();
        }
    }
