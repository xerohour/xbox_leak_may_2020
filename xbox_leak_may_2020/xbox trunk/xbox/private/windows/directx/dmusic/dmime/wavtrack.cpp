                ...
                pSD->apCurrentWave[dwIndex] = pSD->apCurrentWave[dwIndex]->GetNext() )
            {
                ...
                if( !fMute && dwItemVariations )
                {
                        ...
                        (DMUS_PMSG**)&pWave )))
                    {
                        ...
                        {
                            // We need to convert the start offset from clock time to sample time. 
                            if (rtStartOffset)
                            {
                                WAVEFORMATEX WaveFormat;
                                rItem.m_pWave->GetFormat(&WaveFormat,sizeof(WaveFormat),NULL);
                                if (WaveFormat.nSamplesPerSec)
                                {
                                    rtStartOffset *= WaveFormat.nSamplesPerSec;
                                    rtStartOffset /= 10000000;
                            ...
                            pWave->dwStartOffset = (DWORD) rtStartOffset;
                            pWave->lVolume = rItem.m_lVolume + lPartVolume + m_lVolume;
                            if (rItem.m_wVolumeRange)
                            {
                                long lRand = dm_rand() % (2 * rItem.m_wVolumeRange);
                            ...
                            {
                                long lRand = dm_rand() % (2 * rItem.m_wPitchRange);
                                lRand -= rItem.m_wPitchRange;
                                pWave->lPitch += lRand;
                            }
                            pWave->bFlags = (BYTE)(rItem.m_dwFlags & 0xff);
                            pWave->dwLoopStart = rItem.m_dwLoopStart;
                            pWave->dwLoopEnd = rItem.m_dwLoopEnd;
                            // If this isn't looping, let the duration be doubled so one shots will always play through. 
                            if (!pWave->dwLoopEnd)
                            {
                                rtDuration *= 2;
                            }
