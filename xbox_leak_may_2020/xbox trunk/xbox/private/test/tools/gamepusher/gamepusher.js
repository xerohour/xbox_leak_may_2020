...
// latest build and then launches the GamePusher app.

// get command line arguments
var args = WScript.Arguments;

// if wrong number of arguments, show usage message and quit
if(args.length < 2 || args.length > 3)
{
    WScript.Echo("Usage: cscript gamepusher.js sourcedir destinationdir [rebootapp]\n");
    WScript.Quit();
}

// create new .INI file and overwrite the old one
var filesystem = WScript.CreateObject("Scripting.FileSystemObject");
var inifile = filesystem.CreateTextFile("gamepusher.ini", true);

// specify settings
inifile.WriteLine("[Settings]");
inifile.WriteLine("NumThreads=5");
inifile.WriteLine("QueryTime=30");
inifile.WriteLine("FileSleep=5");
inifile.WriteLine("PacketSleep=2");
inifile.WriteLine("XbAppSrc=\\\\xstress\\stress\\GamePusher\\XbServer.xbe");
inifile.WriteLine("XbAppDst=xE:\\GamePusher\\XbServer.xbe");
if(args.length == 3)
{
    inifile.WriteLine("RebootApp=" + args(2));
}
inifile.WriteLine("RecoveryOn=0");
...
WshShell.Run("%comspec% /c dir \/A-D \/B \/S " + args(0) + " >> temp.txt", 0, true);

// sleep for 1 sec to make sure temp.txt exists before we try to open it
WScript.Sleep(1000);
var tempfile = filesystem.OpenTextFile("temp.txt", 1);

// construct regular expression for parsing filenames in source dir
var srcstr = args(0);
srcstr = srcstr.replace(/\\/g, "\\\\");
srcstr = srcstr + "(.*)";
var srcdir = new RegExp(srcstr);

// specify destination dir for each file
while(!tempfile.AtEndOfStream)
{
    var line = tempfile.ReadLine();
    
    result = line.match(srcdir);
    if(result)
    {
        inifile.WriteLine( line + "," + args(1) + result[1]);
    }
}
...
// launch GamePusher
//WshShell.Run("%comspec% /c \\\\xstress\\stress\\GamePusher\\PcClient.exe");
