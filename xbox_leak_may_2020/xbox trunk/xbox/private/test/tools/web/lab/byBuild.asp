...
<%
    ...
    var SQLQuery;
    var where = "";
    var orderBy = "ORDER BY [Current SW Version] DESC, [Machine Name]";
    ...
    var TITLE = "<TABLE><TR><TD><IMG SRC='images/xtextbig.gif' HEIGHT=36></TD><TD> <SPAN STYLE='color:80C040;font:italic bold 30pt Arial'>By Build</SPAN></TD></TABLE>";
%>
<HTML>
<HEAD>
...
while(!records.EOF)
    ...
    Response.Write("\n\n\n\n<TABLE WIDTH=100% CELLPADDING=0 CELLSPACING=0 STYLE='border-top:1px solid #7CB021'><TD STYLE='cursor:hand' onClick='ExpandCollapse(BLOCK" + currentBuild + ");'><H3>" + currentBuild + "</H3></TD><TD STYLE='text-align:right'></TD></TABLE>");
    %>

<BLOCKQUOTE ID=BLOCK<%=currentBuild%>>
...
<%
    while(!records.EOF)
        ...
        if((""+records("Current SW Version")).substr(0,4) != currentBuild) break;
