...
<%
    ...
    if(Request("SUBMIT").Count >= 1)
        ...
        while(!records.EOF)
            {
            if(Request("admin").Count >= 1 || RTrim(records("Owner")).toUpperCase() == currentUser)
                {
...
<CENTER><h2>Release : May</h2></CENTER>
...
<%
        while(!records.EOF)
            ...
            if(RTrim(records("Owner")).toUpperCase() != currentUser) disabled = "DISABLED";
