...
//******************************************************************************
_TVERTEX::_TVERTEX(const D3DXVECTOR3& v, float _tu, float _tv)
{
    vPosition = v;
    fRHW = 0.1f;
    tu = _tu;
...
//
//******************************************************************************
BOOL CTextr1::Create(CDisplay* pDisplay) {
...
//
//******************************************************************************
int CTextr1::Exhibit(int *pnExitCode) {
...
BOOL CTextr1::Prepare() {
    ...
    memset(m_ptq, 0, TEX_DIM_Y * TEX_DIM_X * sizeof(TEXQUAD));
    ...
    for (i = 0, fY = 0.0f; i < TEX_DIM_Y; i++, fY += fTileHeight) {
        for (j = 0, fX = 0.0f; j < TEX_DIM_X; j++, fX += fTileWidth) {
            m_ptq[i * TEX_DIM_X + j].pr[0] = TVERTEX(D3DXVECTOR3(fXOffset + fX, fY + fTileHeight, 0.5f), 0.0f, 1.0f);
            m_ptq[i * TEX_DIM_X + j].pr[1] = TVERTEX(D3DXVECTOR3(fXOffset + fX, fY, 0.5f), 0.0f, 0.0f);
            m_ptq[i * TEX_DIM_X + j].pr[2] = TVERTEX(D3DXVECTOR3(fXOffset + fX + fTileWidth, fY, 0.5f), 1.0f, 0.0f);
            m_ptq[i * TEX_DIM_X + j].pr[3] = TVERTEX(D3DXVECTOR3(fXOffset + fX + fTileWidth, fY + fTileHeight, 0.5f), 1.0f, 1.0f);
...
BOOL CTextr1::Initialize() {
    ...
    if (!LoadResourceData(TEXT("texture.bmp"), (LPVOID*)&fp, &dwSize)) {
        DebugString(TEXT("LoadResourceData failed"));
        return FALSE;
    }
    ...
    for (i = 0; i < TEX_DIM_Y; i++) {
        ...
        for (j = 0; j < TEX_DIM_X; j++) {
            ...
            hr = m_pDevice->CreateTexture(1, 1, 1, 0, D3DFMT_R5G6B5, POOL_DEFAULT, &pd3dt);
            if (FAILED(hr)) {
                UnloadResourceData(TEXT("texture.bmp"));
                return FALSE;
            }
    ...
    for (i = 0; i < TEX_DIM_Y; i++) {
        ...
        for (j = 0; j < TEX_DIM_X; j++) {
            ...
            m_pDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2,
                    m_ptq[i * TEX_DIM_X + j].pr, sizeof(TVERTEX));
        }
    }
...
//
//******************************************************************************
BOOL CTextr1::WndProc(LRESULT* plr, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
