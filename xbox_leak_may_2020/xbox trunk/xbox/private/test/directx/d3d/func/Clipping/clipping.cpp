...
extern "C" BOOL PREPEND_MODULE(_ExhibitScene)(CDisplay* pDisplay, int* pnExitCode) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        switch (i) {
            case 0:     pClipping = new TFan;       break;
            case 1:     pClipping = new TStrip;     break;
            case 2:     pClipping = new Triangle;   break;
            case 3:     pClipping = new Line;       break;
            case 4:     pClipping = new LStrip;     break;
            case 5:     pClipping = new Point;      break;
        }
...
RENDERPRIMITIVEAPI Clipping::m_pVBAPIList[2] =
{
    RP_DPVB,                    // DrawPrimitive (FVF shader)
    RP_DIPVB,                   // DrawIndexedPrimitive (FVF shader)
};

DWORD Clipping::m_dwVBAPIListSize = 2;
...
UINT Clipping::TestInitialize(void) {
...
bool Clipping::ClearFrame(void)
{
    return Clear(0);
//    return true;
}

bool Clipping::ExecuteTest(UINT uTestNumber) {
...
void Clipping::SceneRefresh(void) {
	RECT				rect;
...
bool Clipping::ProcessFrame(void) { 
...
bool Clipping::TestTerminate(void) 
{
//    RELEASE(m_pMaterial);
//    RELEASE(m_pLight);
    for (int i = 0; i < 8; i++) {
