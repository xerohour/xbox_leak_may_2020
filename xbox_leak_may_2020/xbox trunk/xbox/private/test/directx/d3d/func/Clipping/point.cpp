...
RENDERPRIMITIVEAPI Point::m_pDX8APIList[2] =
{
    RP_DP,                      // DrawPrimitive
    RP_DPS,                     // DrawPrimitiveStrided
};

DWORD Point::m_dwDX8APIListSize = 2;
...
UINT Point::TestInitialize()
{
    ...
    if (GetStartupContext() & TSTART_STRESS) {
        ...
        SetAPI(m_pVBAPIList, m_dwVBAPIListSize, 2);
    }
    else {
#endif
...
bool Point::TestTerminate(void)
{
    ...
    return Clipping::TestTerminate();
}


bool Point::GeneratePointList(void) {
	int		minX = -40;
	int		maxX =  40;
	int		minY = -40;
	int		maxY =  40;
