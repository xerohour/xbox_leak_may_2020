...
bool VShader::ExecuteTest(UINT uCurrentTest)
{
    ...
    m_dwCurrentTestNumber = (DWORD)uCurrentTest;
    m_dwCurrentShader = (m_dwCurrentTestNumber-1)%m_dwShaders;
...
void VShader::SceneRefresh(void)
{
    ...
    if (BeginScene())
    {
        ...
        if (!(GetStartupContext() & TSTART_STRESS)) {
            ...
            m_pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, m_dwVertices, 0, m_dwIndices/12);
            m_pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, m_dwVertices, m_dwIndices / 4, m_dwIndices/12);
            m_pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, m_dwVertices, m_dwIndices / 2, m_dwIndices/12);
            m_pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, m_dwVertices, 3 * m_dwIndices / 4, m_dwIndices/12);
        }
#endif
