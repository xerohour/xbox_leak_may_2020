...
//******************************************************************************
_TVERTEX3::_TVERTEX3(const D3DXVECTOR3& v, float _tu, float _tv)
{
    vPosition = v;
    fRHW = 0.1f;
    tu = _tu;
...
//
//******************************************************************************
BOOL CTextr3::Create(CDisplay* pDisplay) {
...
//
//******************************************************************************
int CTextr3::Exhibit(int *pnExitCode) {
...
BOOL CTextr3::Prepare() {
    ...
    for (i = 0, fY = 0.0f; i < TEX_DIM_Y; i++, fY += fTileHeight) {
        for (j = 0, fX = 0.0f; j < TEX_DIM_X; j++, fX += fTileWidth) {
            m_ptq[i * TEX_DIM_X + j].pr[0] = TVERTEX3(D3DXVECTOR3(fXOffset + fX, fY + fTileHeight, 0.5f), 0.0f, 1.0f);
            m_ptq[i * TEX_DIM_X + j].pr[1] = TVERTEX3(D3DXVECTOR3(fXOffset + fX, fY, 0.5f), 0.0f, 0.0f);
            m_ptq[i * TEX_DIM_X + j].pr[2] = TVERTEX3(D3DXVECTOR3(fXOffset + fX + fTileWidth, fY, 0.5f), 1.0f, 0.0f);
            m_ptq[i * TEX_DIM_X + j].pr[3] = TVERTEX3(D3DXVECTOR3(fXOffset + fX + fTileWidth, fY + fTileHeight, 0.5f), 1.0f, 1.0f);
...
BOOL CTextr3::Initialize() {
    ...
    for (i = 0; i < TEX_DIM_Y; i++) {
        ...
        for (j = 0; j < TEX_DIM_X; j++) {
            ...
            pd3dt = (CTexture8*)CreateTexture(m_pDevice, 1 << j, 1 << i, D3DFMT_R5G6B5, TTYPE_MIPMAP);
            if (!pd3dt) {
                UnloadResourceData(TEXT("texture.bmp"));
                return FALSE;
            }
...
BOOL CTextr3::Render() {
    ...
    m_pDevice->SetVertexShader(FVF_TVERTEX3);
    ...
    for (i = 0; i < TEX_DIM_Y; i++) {
        ...
        for (j = 0; j < TEX_DIM_X; j++) {
            ...
            m_pDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2,
                    m_ptq[i * TEX_DIM_X + j].pr, sizeof(TVERTEX3));
        }
    }
...
//
//******************************************************************************
BOOL CTextr3::WndProc(LRESULT* plr, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
