...
//DebugString(TEXT("seeding pseudo-random number generator with 0x%X"), dwSeed);
    ...
    m_bEMC = (BOOL)GetProfileInt(szName, TEXT("TestEMC"), 0);
...
BOOL CClockwork::Initialize() {
    ...
    m_pd3dtBackground = (CTexture8*)CreateTexture(m_pDevice, TEXT("backcogs.bmp"), D3DFMT_A8R8G8B8, TTYPE_TEXTURE, NULL, 1.0f, COLORKEY_NONE, D3DPOOL_MANAGED, 256, 256);
    ...
    if (m_bEMC) {
        ...
        m_pd3dtStripes = (CTexture8*)CreateTexture(m_pDevice, 641, 1, D3DFMT_LIN_R5G6B5);
        if (!m_pd3dtStripes) {
            return FALSE;
        }

        m_pd3dtStripes->LockRect(0, &d3dlr, NULL, 0);
        pwTexel = (LPWORD)d3dlr.pBits;
        for (i = 0; i < 641; i++) {
            pwTexel[i] = (i % 2) ? 0xFFFF : 0;
        }
        m_pd3dtStripes->UnlockRect(0);

        m_prStripes[0] = TLVERTEX(D3DXVECTOR3(-0.5f, (float)(m_pDisplay->GetHeight()) - 0.5f, 0.9999f), 0.01f, RGBA_MAKE(255,255,255,255), RGB_MAKE(0,0,0), 0.0f, 1.0f);
        m_prStripes[1] = TLVERTEX(D3DXVECTOR3(-0.5f, -0.5f, 0.9999f), 0.01f, RGBA_MAKE(255,255,255,255), RGB_MAKE(0,0,0), 0.0f, 0.0f);
        m_prStripes[2] = TLVERTEX(D3DXVECTOR3((float)(m_pDisplay->GetWidth()) - 0.5f, -0.5f, 0.9999f), 0.01f, RGBA_MAKE(255,255,255,255), RGB_MAKE(0,0,0), 640.0f, 0.0f);
        m_prStripes[3] = TLVERTEX(D3DXVECTOR3((float)(m_pDisplay->GetWidth()) - 0.5f, (float)(m_pDisplay->GetHeight()) - 0.5f, 0.9999f), 0.01f, RGBA_MAKE(255,255,255,255), RGB_MAKE(0,0,0), 640.0f, 1.0f);
    }
...
//        m_pDevice->SetRenderState(D3DRS_COLORWRITEENABLE, FALSE);
    ...
    if (m_bEMC) {
        m_pDevice->SetRenderState(D3DRS_ZENABLE, D3DZB_FALSE);
//        m_pDevice->SetRenderState(D3DRS_COLORWRITEENABLE, D3DCOLORWRITEENABLE_ALL);
        m_pDevice->SetTexture(0, m_pd3dtStripes);
        ...
        m_prStripes[0].u0 = 0.0f;
        m_prStripes[1].u0 = 320.0f;
        m_prStripes[2].u0 = 640.0f;
        m_prStripes[3].u0 = 320.0f;
        m_prStripes[0].v0 = 0.5f;
        m_prStripes[1].v0 = 0.0f;
        m_prStripes[2].v0 = 0.5f;
        m_prStripes[3].v0 = 1.0f;

        m_pDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2, m_prStripes, sizeof(TLVERTEX));

        m_pDevice->SetRenderState(D3DRS_STENCILFUNC, D3DCMP_EQUAL);

        if ((UINT)m_fFrame % 2) {
            m_prStripes[0].u0 = 0.0f;
            m_prStripes[1].u0 = 0.0f;
            m_prStripes[2].u0 = 640.0f;
            m_prStripes[3].u0 = 640.0f;
        }
        else {
            m_prStripes[0].u0 = 1.0f;
            m_prStripes[1].u0 = 1.0f;
            m_prStripes[2].u0 = 641.0f;
            m_prStripes[3].u0 = 641.0f;
        }
        m_prStripes[0].v0 = 1.0f;
        m_prStripes[1].v0 = 0.0f;
        m_prStripes[2].v0 = 0.0f;
        m_prStripes[3].v0 = 1.0f;
