...
D3DVERTEX	Triangle::m_pTriangleVertices[24] = {
...
UINT Triangle::TestInitialize()
{
    ...
    if (GetStartupContext() & TSTART_STRESS) {
        ...
        m_dwTotalTests = 8 * m_dwTotalAPIs * 2 * 7 * 3; // (?)
#endif
        SetAPI(m_pVBAPIList, m_dwVBAPIListSize, 2*8);
    }
    else {
        m_dwTotalAPIs = m_dwRPFullDX8APIListSize * 2;
#ifdef TESTTYPE_WIREFRAME
...
bool Triangle::TestTerminate(void)
{
    if (m_pRenderVertices)
    {
        delete [] m_pRenderVertices;
...
bool Triangle::ResolveTestNumber()
{
