...
#else
    m_pd3dtMirror = (CTexture8*)CreateTexture(m_pDevice, 512, 512, D3DFMT_LIN_X8R8G8B8, TTYPE_TARGET);
#endif // UNDER_XBOX
    if (!m_pd3dtMirror) {
        return FALSE;
    }
...
#ifndef UNDER_XBOX
    hr = m_pDevice->CreateDepthStencilSurface(512, 512, D3DFMT_D24S8, D3DMULTISAMPLE_NONE, &m_pd3dsZMirror);
#else
...
HRESULT CMirror::SetReflectionTextureCoords(PMVERTEX prVertices, UINT uNumVertices) {
    ...
    for (i = 0; i < uNumVertices; i++) {
		...
		prVertices[i].u0 = fX * 512.0f;
        prVertices[i].v0 = (-fY + 1.0f) * 512.0f;
    }
