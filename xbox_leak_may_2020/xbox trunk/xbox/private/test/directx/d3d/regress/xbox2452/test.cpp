...
BOOL CReproTest::Create(HINSTANCE hInstance) {
    ...
    plrVertices[1] = LVERTEX(D3DXVECTOR3(-10.0f,  10.0f, 0.0f), RGBA_MAKE(255, 255, 255, 255), 0, 0.0f, 0.0f);
    plrVertices[2] = LVERTEX(D3DXVECTOR3( 10.0f,  10.0f, 0.0f), RGBA_MAKE(255, 255, 255, 255), 0, 1.0f, 0.0f);
    plrVertices[3] = LVERTEX(D3DXVECTOR3( 10.0f, -10.0f, 0.0f), RGBA_MAKE(255, 255, 255, 255), 0, 1.0f, 1.0f);

    hr = m_pDevice->CreateVertexBuffer(4 * sizeof(LVERTEX), 0, FVF_LVERTEX, D3DPOOL_DEFAULT, &m_pd3dr);
    if (ResultFailed(hr, TEXT("IDirect3DDevice8::CreateVertexBuffer"))) {
        return FALSE;
    }

    hr = m_pd3dr->Lock(0, 4 * sizeof(LVERTEX), &pData, 0);
    if (ResultFailed(hr, TEXT("IDirect3DVertexBuffer8::Lock"))) {
        return FALSE;
    }

    memcpy(pData, plrVertices, 4 * sizeof(LVERTEX));
    ...
    m_pDevice->CreateTexture(256, 256, 1, 0, D3DFMT_DXT1, D3DPOOL_MANAGED, &m_pd3dt);

    m_pd3dt->LockRect(0, &d3dlr, NULL, 0);
    for (i = 0, pwPixel = (LPWORD)d3dlr.pBits; i < (256 >> 2); i++) {
        for (j = 0; j < 256; j+=4) {
            pwPixel[j]   = 0xF800;
            pwPixel[j+1] = 0x001F;
            pwPixel[j+2] = 0x1E78;
