...
#endif // UNDER_XBOX
{
    ...
    if (!pRTest->Create(hInstance, RGB_MAKE(0, 255, 0))) {
    ...
    // Initialize the display
    if (!pRTest->Create(hInstance, RGB_MAKE(0, 0, 255))) {
        DebugString(TEXT("CDisplay::Create failed"));
        delete pRTest;
        ReleaseHeap();
        return 0;
...
//******************************************************************************
BOOL CReproTest::Create(HINSTANCE hInstance, D3DCOLOR c) {
    ...
    prTriangle[0] = TLVERTEX(D3DXVECTOR3(320.0f, 0.0f, 0.9999f), 0.01f, c, RGB_MAKE(0,0,0), 0.0f, 1.0f);
    prTriangle[1] = TLVERTEX(D3DXVECTOR3(640.0f, 480.0f, 0.9999f), 0.01f, c, RGB_MAKE(0,0,0), 0.0f, 0.0f);
    prTriangle[2] = TLVERTEX(D3DXVECTOR3(0.0f, 480.0f, 0.9999f), 0.01f, c, RGB_MAKE(0,0,0), 1.0f, 0.0f);

    hr = m_pDevice->CreateVertexBuffer(3 * sizeof(TLVERTEX), 0, FVF_TLVERTEX, D3DPOOL_DEFAULT, &m_pd3dr);
    if (ResultFailed(hr, TEXT("IDirect3DDevice8::CreateVertexBuffer"))) {
        return NULL;
    }

    hr = m_pd3dr->Lock(0, 3 * sizeof(TLVERTEX), &pData, 0);
    if (ResultFailed(hr, TEXT("IDirect3DVertexBuffer8::Lock"))) {
        return NULL;
    }

    memcpy(pData, prTriangle, 3 * sizeof(TLVERTEX));
