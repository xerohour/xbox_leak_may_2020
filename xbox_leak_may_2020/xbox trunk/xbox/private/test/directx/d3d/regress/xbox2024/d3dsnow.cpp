...
BOOL CSnow::Initialize() {
    ...
    m_ppd3drSnowMesh = (CVertexBuffer8**)MemAlloc(sizeof(CVertexBuffer8*) * m_uNumCells);
    if (!m_ppd3drSnowMesh) {
        return FALSE;
    }
    memset(m_ppd3drSnowMesh, 0, m_uNumCells * sizeof(CVertexBuffer8*));
    for (i = 0; i < m_uNumCells; i++) {
...
*/
    ...
    m_pDevice->SetTexture(3, m_pd3dtSnowFlake);

    m_pDevice->SetRenderState(D3DRS_POINTSIZE, F2DW(m_fFlakeDim));
    m_pDevice->SetRenderState(D3DRS_POINTSCALE_A, F2DW(0.0f));
    m_pDevice->SetRenderState(D3DRS_POINTSCALE_B, F2DW(1.0f));
    m_pDevice->SetRenderState(D3DRS_POINTSCALE_C, F2DW(0.5f));
    m_pDevice->SetRenderState(D3DRS_POINTSCALEENABLE, TRUE);
    m_pDevice->SetRenderState(D3DRS_POINTSPRITEENABLE, TRUE);

m_pDevice->SetRenderState(D3DRS_LIGHTING, FALSE);
SetColorStage(m_pDevice, 3, D3DTA_TEXTURE, D3DTA_DIFFUSE, D3DTOP_SELECTARG1);
