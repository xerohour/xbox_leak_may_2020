...
BOOL CReproTest::Create(HINSTANCE hInstance) {
    ...
    m_pDevice->CreateTexture(256, 256, 1, 0, D3DFMT_P8, 0, &pd3dt);
    pd3dt->GetLevelDesc(0, &SrcDesc);

    pd3dt->LockRect(0, &SrcLock, NULL, 0);
    ...
    DebugString(TEXT("Size: %d, Width: %d, Height: %d, Pitch: %d, Bytes per pixel: %d"), SrcDesc.Size, SrcDesc.Width, SrcDesc.Height, SrcLock.Pitch, XGBytesPerPixelFromFormat(SrcDesc.Format));
