...
//    d3dpp.AutoDepthStencilFormat            = D3DFMT_UNKNOWN_D24S8;
    ...
    // Create the device
    hr = m_pd3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &d3dpp, &m_pDevice);
...
//hr = m_pDevice->CreateTexture(256, 256, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &pd3dt);
    hr = D3DXCreateTextureFromFileEx(m_pDevice, 
                        TEXT("chess.bmp"), 256, 256, 1, 0, 
                        D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                        D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR,
                        0, NULL, &pd3dt);
    if (ResultFailed(hr, TEXT("D3DXCreateTextureFromFileEx"))) {
        return FALSE;
    }

    hr = pd3dt->LockRect(0, &d3dlr, NULL, 0);
    if (ResultFailed(hr, TEXT("Lock"))) {
        return FALSE;
    }
    pd3dt->UnlockRect(0);
    if (ResultFailed(hr, TEXT("Unlock"))) {
        return FALSE;
    }
