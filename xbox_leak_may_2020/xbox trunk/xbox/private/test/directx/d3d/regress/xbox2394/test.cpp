...
BOOL CTest::Initialize() {
    ...
    m_pd3dt1 = (CTexture8*)CreateTexture(m_pDevice, 256, 256, D3DFMT_A8R8G8B8);
    if (!m_pd3dt1) {
        return FALSE;
    }

    m_pd3dt2 = (CTexture8*)CreateTexture(m_pDevice, TEXT("directx.bmp"));
    if (!m_pd3dt2) {
        return FALSE;
    }

    m_pd3dt1->LockRect(0, &d3dlr, NULL, 0);
    ...
    for (i = 0; i < 256; i++) {
        ...
        for (j = 0; j < 256; j++) {
            ...
            pdwPixel[j] = RGBA_MAKE(i, (BYTE)((255.0f - (float)j) / 2.0f), 255 - i, (BYTE)((float)j / 2.0f));
        }

        pdwPixel += d3dlr.Pitch >> 2;
    }

    m_pd3dt1->UnlockRect(0);
