...
BOOL CTest::Initialize() {
    ...
    m_prVertices[0] = LVERTEX(D3DXVECTOR3(-10.0f,-10.0f, 0.0f), RGBA_MAKE(128, 128, 128, 255), 0, 0.0f, 1.0f);
    m_prVertices[1] = LVERTEX(D3DXVECTOR3(-10.0f, 10.0f, 0.0f), RGBA_MAKE(128, 128, 128, 255), 0, 0.0f, 0.0f);
    m_prVertices[2] = LVERTEX(D3DXVECTOR3( 10.0f, 10.0f, 0.0f), RGBA_MAKE(128, 128, 128, 255), 0, 1.0f, 0.0f);
    m_prVertices[3] = LVERTEX(D3DXVECTOR3( 10.0f,-10.0f, 0.0f), RGBA_MAKE(128, 128, 128, 255), 0, 1.0f, 1.0f);

    hr = m_pDevice->CreatePixelShaderAsm(szShader, strlen(szShader), &m_dwPShader);
    if (ResultFailed(hr, TEXT("IDirect3DDevice8::CreatePixelShader"))) {
        return FALSE;
    }

    m_pd3dt = (CTexture8*)CreateTexture(m_pDevice, TEXT("wall.bmp"));
    if (!m_pd3dt) {
        return FALSE;
    }
