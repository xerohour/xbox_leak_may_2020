...
SHE         SHLoadDllEx( LSZ, BOOL, ULONG TimeStamp, ULONG CheckSum,
						 ULONG LoadAddress, HEXE* lphexe);
...
SHE         SHGetExeTimeStamp( LSZ, ULONG* TimeStamp, ULONG* CheckSum );
...
BOOL        SHIsThunk ( HSYM, THUNK_ORDINAL*, ULONG* length);
HSYM        SHFindSLink32 ( PCXT );

LSZ         SHGetSrcPath(HMOD);
LSZ         SHGetObjPath(HMOD);
LSZ         SHGetCompileEnv(HMOD);
BOOL        SHModSupportsEC(HMOD);
BOOL        SHExeSupportsEC(HEXE);
BOOL        SHModLinkedFromLib(HMOD);

BOOL        SHModFromHmod( HMOD, PHANDLE );
BOOL        SHFlushModSyms( HMOD hmod );
void        SHUseAltPdb ( PFNPDBOpenValidateEx );
...
extern PUCHAR ConvertSymbolsForImage2(
                       HANDLE      hFile,
                       char *      fname,
					   SHE *       pSheResult
    );
