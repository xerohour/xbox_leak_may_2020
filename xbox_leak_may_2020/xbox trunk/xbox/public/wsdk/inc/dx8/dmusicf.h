...
(
    ...
    (
        ...
        [<mrkr-ck>]     // List of markers in Part
        [<rsln-ck>]     // List of variation resolutions in Part
        [<anpn-ck>]     // List of resolution anticipations in Part
    )
    ...
    (
        ...
        [<DMBD-form>]   // Band to be associated with the pattern (for motifs)
...
(
    ...
    [<dssl-list>]   // Optional DSound Sink configuration
    [<pcsl-list>]   // Optional list of port configurations
    [<dbfl-list>]   // Optional set of Dsound buffer descriptors
)
*/
...
LIST
(
    'pcsl'          // List of port configurations
...
(
    ...
    (
        ...
        <GUID>...                           // Set of buffers that they all connect to.
    )
)
...
LIST
{
    ...
    LIST
    {
        ...
        {
            'wavi'
            <wave-list>     //  Each wave is encapsulated in a list
        }
...
/*  File io for DirectMusic Container file. This embeds a set of related files. And,
    in turn, it can be embedded within a segment file.
*/
...
(
    ...
    LIST
    (
        'srsl'          // List of segment references
    ...
    // <trsl-list>      // Track reference container
    (
        'trsl'
...
(
    ...
    <tims-ck>       // Array of time signatures
)
...
(
    ...
    [<vals-ck>]     // Array of start points
    [<play-ck>]     // Array of play start markers
)
...
(
    ...
    (
        ...
        (
            'lseg'
            <sgih-ck>
            <DMRF-list>    // Link to a segment file.
...
/*
    ...
    (
        ...
        LIST
        (
            <scre-list>       // Description of an event
        )
...
/*
    ...
    (
        ...
        LIST
        (
            <lyre-list>       // Description of an event
        )
    ...
    (
        ...
        (
            ...
            (
                ...
                <prcp-ck>       // array of control points
            )
...
{
    DWORD        dwPlaymode;       /* Playmode for the generated pattern track */   
} DMUS_IO_MELFORM;
...
(
    ...
    <fxls-list>	    // List of FX descriptors
)
	...
	'bsid'
	(
		<DSOUND_IO_DSBUSID>  // Where the array size is determined by the chunk size 
	)
    ...
    LIST
    (
        'fxls'          // List of DMO creation parameter blocks
